    ...
    <div className="min-h-screen bg-background px-5 pt-14 pb-4">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <p className="text-cyan-400 text-sm font-semibold uppercase tracking-widest">Geolocalização</p>
        </div>
      ...
      <div className="glass-card rounded-2xl p-5 mb-4">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <span className="text-cyan-400 text-xs font-medium">GPS Ativo</span>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          style={{
            background: 'linear-gradient(135deg, rgba(0,100,150,0.2) 0%, rgba(0,50,100,0.3) 100%)',
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative">
              {/* Outer ring */}
              <div className="absolute inset-0 rounded-full bg-cyan-400/20 animate-ping scale-150" />
              <div className="absolute inset-0 rounded-full bg-cyan-400/30 scale-125" />
              <div className="w-8 h-8 rounded-full bg-cyan-400/80 flex items-center justify-center shadow-[0_0_20px_rgba(0,245,255,0.6)]">
                <div className="w-3 h-3 rounded-full bg-white" />
              </div>
            </div>
          </div>
          <div className="absolute bottom-3 left-3 px-2 py-1 rounded-lg bg-black/40 backdrop-blur-sm">
            <p className="text-cyan-400 text-xs font-mono">📍 Localização ativa</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-secondary/50">
          ...
          <p className="text-muted-foreground text-sm">Localização sendo monitorada em tempo real</p>
        </div>
      </div>
      ...
      {zones.length > 0 && (
        <div className="glass-card rounded-2xl p-4 mb-4">
          ...
          <div className="space-y-2">
            {zones.map(zone => (
              <div key={zone.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-secondary/40">
                ...
                <div className="flex items-center gap-1">
                  <span className={`w-2 h-2 rounded-full ${zone.is_active ? 'bg-green-400 animate-pulse' : 'bg-gray-500'}`} />
                  <span className={`text-xs font-medium ${zone.is_active ? 'text-green-400' : 'text-gray-500'}`}>
        ...
        <div className="glass-card rounded-2xl p-4">
          ...
          <div className="space-y-2">
            {recentWithLocation.slice(0, 4).map(alert => (
              <div key={alert.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-secondary/40">
